...
export async function handleHelp(engine: EngineCore, noun?: string): Promise<void> {
  ...
  const lines = [
    ...
    `  Looking:  ${rt.keyword('look/l')}, ${rt.keyword('look [thing]')}, ${rt.keyword('smell')}, ${rt.keyword('listen')}`,
    ...
    `  Social:   ${rt.keyword('talk [npc]')}, ${rt.keyword('trade [npc]')}`,
    `  Info:     ${rt.keyword('score')}, ${rt.keyword('inventory')}, ${rt.keyword('equipment')}, ${rt.keyword('map')}, ${rt.keyword('journal')}, ${rt.keyword('help')}`,
    `  System:   ${rt.keyword('save')}, ${rt.keyword('rest')}`,
    BAR,
    '',
